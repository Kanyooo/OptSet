...
from models.primitives import NormPrimitives, QuadraticPrimitives
...
class SOCICNN(nn.Module):
    def __init__(
        ...
        num_quad_terms: int = 1,
        num_norm_terms: int = 1,
        quad_term_dim: int | None = None,
        norm_term_dim: int | None = None,
    ):
        super().__init__()
    ...
    def forward(self, x):
        return self.backbone(x) + self.quad(x) + self.norm(x)
